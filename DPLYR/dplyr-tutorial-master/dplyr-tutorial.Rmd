...
head(flights)
flights[1] #First Column
flights[1,] #First Row
flights[1,2] #First row and second column
flights[,2] # Second column
flights[0,2] #Nothing
...
flights %>%
    ...
    filter(DepDelay > 60)
head(flights)
#Q: Flights made by Flight number 428 in the month of January
nrow(flights %>% select(FlightNum, TailNum, Month) %>% filter(FlightNum==428 & Month==1))
...
head(flights)
#Identify flights with lowest and highest AirTime 
d1=flights %>% select(FlightNum,TailNum,AirTime) %>% arrange(desc(AirTime))
d1[1,1] 
d1[1,2]
...
head(flights)
#Q: Find the total airtime of each Carrier
flights %>% group_by(UniqueCarrier) %>% summarise(Total_AirTime=sum(AirTime,na.rm = TRUE))
```
...
# For each carrier, calculate total flights cancelled, diverted
flights %>% group_by(UniqueCarrier) %>% summarise_each(funs(sum),Cancelled, Diverted)

head(flights)
#Find all FlightNum to the destinatiom
```
...
#for each carrier find the number of planes and the number of destinations served
flights%>%group_by(UniqueCarrier)%>%summarise(plane_count=n_distinct(TailNum),dest_served=n_distinct(Dest))
```
...
flights %>% group_by(UniqueCarrier) %>% select(DayofMonth,Month,DepDelay) %>% slice_max(DepDelay,n=2) %>% arrange(UniqueCarrier,desc(DepDelay))
...
head(flights)
flights %>% group_by(Month) %>% summarise(flight_count=n()) %>% mutate(change_perc= round((flight_count-lag(flight_count))/lag(flight_count),2)*100)
